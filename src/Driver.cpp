...
bool generate_vector(VectorPass *vpass, FILE *pjl_file) {
	...
	int freq = vpass->lconf->vector_freq;
	printf("%d", vpass->lconf->vector_freq);
	printf("%d", freq);
	fprintf(pjl_file, V_FREQUENCY, (int)(vpass->lconf->vector_freq));
	fprintf(pjl_file, SEP);
	fprintf(pjl_file, V_POWER, (int)(vpass->lconf->vector_power));
	fprintf(pjl_file, SEP);
	fprintf(pjl_file, V_SPEED, (int)(vpass->lconf->vector_speed));
	fprintf(pjl_file, SEP);
	...
	for (it = skins.begin(); it != skins.end(); it++) {
		...
		for (it_s = skin->segments.begin(); it_s != skin->segments.end(); it_s++) {
			...
			if (ls->getPower() != power) {
				...
				if (vpass->lconf->vector_speed && vpass->lconf->vector_speed
						< 100) {
			...
			fprintf(pjl_file, "%d,%d", vpass->lconf->basex
					+ ls->getStart()->getX() + HPGLX, vpass->lconf->basey
					+ ls->getStart()->getY() + HPGLY);
			fprintf(pjl_file, SEP);
			fprintf(pjl_file, HPGL_PEN_DOWN);
			fprintf(pjl_file, "%d,%d", vpass->lconf->basex
					+ ls->getEnd()->getX() + HPGLX, vpass->lconf->basey
					+ ls->getEnd()->getY() + HPGLY);
			fprintf(pjl_file, SEP);
			first = 0;
		}
	}
...
void Driver::process(LaserJob *job) {
	...
	job->pconf->pjl_filename = new string("/tmp/test.pjl");
	FILE* pjl_file = fopen("/tmp/test.pjl", "w");
	generate_pjl(job, pjl_file);
	fclose(pjl_file);
	PrinterConnection pc(new string("10.20.30.27"), PRINTER_MAX_WAIT);

	if (!pc.connect() || !pc.send(job->pconf)) {
