...
bool execute_ghostscript(char *filename_bitmap, char *filename_eps,
		char *filename_vector, char *bmp_mode, int resolution, int height,
		int width) {
	char buf[8192];
	sprintf(
			buf,
			"/usr/bin/gs -q -dBATCH -dNOPAUSE -r%d -g%dx%d -sDEVICE=%s -sOutputFile=%s %s > %s",
			resolution, (width * resolution) / POINTS_PER_INCH, (height
					* resolution) / POINTS_PER_INCH, bmp_mode, filename_bitmap,
...
bool generate_raster(printer_job *pjob, laser_config *lconf, FILE *bitmap_file) {
	...
	while (repeat--) {
		...
		/* Raster power */
		fprintf(pjob->pjl_file, R_POWER, (lconf->raster_mode == 'c'
		...
		/* Raster compression */
		fprintf(pjob->pjl_file, R_COMPRESSION, (lconf->raster_mode == 'c'
				...
				for (pass = 0; pass < passes; pass++) {
					...
					for (y = lconf->height - 1; y >= 0; y--) {
						...
						if (l < h) {
							...
							r++;
							fprintf(pjob->pjl_file, PCL_POS_Y, lconf->basey
									+ offy + y);
							fprintf(pjob->pjl_file, PCL_POS_X, lconf->basex
									+ offx + ((lconf->raster_mode == 'c'
							...
							fprintf(pjob->pjl_file, R_ROW_BYTES, (n + 7) / 8
									* 8);
							r = 0;
							while (r < n)
		...
		fprintf(pjob->pjl_file, "\e*rC"); // end raster
		fputc(26, pjob->pjl_file); // some end of file markers
		fputc(4, pjob->pjl_file);
	}
	return true;
}
			...
			while (fgets((char *) buf, sizeof(buf), vector_file)) {
				if (isalpha(*buf)) {
					...
					case 'C': // close - only makes sense after an "L"
						if (newline == 0 && up == 0 && (lx != sx || ly != sy)) {
							fprintf(pjob->pjl_file, ",%d,%d", lconf->basex
									+ offx + sx + HPGLX, lconf->basey + offy
									+ sy + HPGLY);
						}
						break;
					case 'P': // power
						if (sscanf((char *) buf + 1, "%d", &x) == 1 && x
								!= power) {
							int epower;
							power = x;
							if (!started) {
							...
							if (lconf->vector_speed && lconf->vector_speed
									< 100) {
					...
					case 'L': // line
						...
						if (sscanf((char *) buf + 1, "%d,%d", &y, &x) == 2) {
							fprintf(pjob->pjl_file, "%d,%d", lconf->basex
									+ offx + x + HPGLX, lconf->basey + offy + y
									+ HPGLY);
 ...
 *
 */
bool generate_pjl(printer_job *pjob, laser_config *lconf, FILE *bitmap_file,
		FILE *vector_file) {
	int i;
	...
	/* FIXME unknown purpose. */
	fprintf(pjob->pjl_file, PCL_UNKNOWN_BLAFOO2);
	...
	if (lconf->vector_power) {
		...
		fprintf(pjob->pjl_file, R_COMPRESSION);
		fprintf(pjob->pjl_file, PCL_SECTION_END);
 ...
 */
bool process_print_job_options(printer_job *pjob, laser_config *lconf) {
	char *o = strchr(pjob->options, ' ');
...
int main(int argc, char *argv[]) {
	...
	if (argc == 1) {
		printf(
				"direct epilog \"Unknown\" \"Epilog laser (thin red lines vector cut)\"\n");
		return 1;
	}
	if (argc > 1) {
		pjob.name = argv[1];
	...
	pjob.pjl_file = fopen(filename_pjl, "w");
	if (!pjob.pjl_file) {
		perror(filename_pjl);
		return 1;
	}
	...
	pjob.pjl_file = fopen(filename_pjl, "r");
	if (!pjob.pjl_file) {
		perror(filename_pjl);
		return 1;
	}
