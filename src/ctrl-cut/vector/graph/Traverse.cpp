...
/* REFACTOR
void check_linestrings(StringList::iterator first, StringList::iterator last, DocumentSettings& docSettings) {
  std::set<Segment> segments;

  for(StringList::iterator it = first; it != last; ++it) {
    const SegmentString& string = **it;
    double gdWidth = docSettings[DocumentSettings::WIDTH];
    double gdHeight = docSettings[DocumentSettings::HEIGHT];
