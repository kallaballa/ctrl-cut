...
void PclEncoder::encode(std::ostream &out, Engraving& raster)
{
  ...
  BitmapImage *bitmap = dynamic_cast<BitmapImage*>(raster.sourceImage());
  if (!bitmap) {
    GrayscaleImage *gsimage =  dynamic_cast<GrayscaleImage*>(raster.sourceImage());
    if (gsimage) {
...
void PclEncoder::encodeBitmapTile(Engraving& raster, BitmapImage* tile, std::ostream& out)
{
