 ...
 */
bool
process_queue_options(laser_config *lconf, char *queue_options)
{
...
main(int argc, char *argv[])
{
    ...
    if (!process_queue_options(&lconf, queue_options)) {
    ...
    if(!execute_ghostscript(filename_bitmap,
                            filename_eps,
                            filename_vector,
                            (lconf.raster_mode == 'c') ? "bmp16m" :
                            (lconf.raster_mode == 'g') ? "bmpgray" : "bmpmono",
